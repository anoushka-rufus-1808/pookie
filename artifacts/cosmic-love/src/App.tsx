...
// The illustrated trekking cat (cat3.png, true transparent background), used
// as a recurring decorative motif across the page. Sized to be large and
// clearly noticeable, with a slow idle bob plus subtle tail-flick and blink
// layers so the cat feels alive in each of its scattered instances.
function MiniCat({
  className = "",
  size = "md",
  flip = false,
  ...
  return (
    <div
      className={`animate-cat-float pointer-events-none select-none bg-transparent ${sizeClasses} ${className}`}
      style={{ transform: flip ? "scaleX(-1)" : undefined }}
    >
      <div className="relative w-full h-full bg-transparent animate-tail-flick">
        <img
          src="/cat3.png"
          alt=""
          className="w-full h-full object-contain bg-transparent animate-cat-blink drop-shadow-[0_6px_14px_rgba(0,0,0,0.45)]"
          draggable={false}
        />
      </div>
    </div>
